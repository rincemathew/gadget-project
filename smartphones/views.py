...
from wearables.models import WearModelName
from smartphones.models import MobileName, MobileVariant, BrandName
from articles.models import Articles
from . import listcontent


def home_view(request):
    budget_mobiles = MobileVariant.objects.filter(Q(mobileGeneral__status='Available') & Q(mobileGeneral__is_available=True) & Q(mobileNames__phone_type='SMARTPHONE')).order_by("-mobileGeneral__release_date")[:10]
    ...
    wear_wear_list = WearModelName.objects.filter(Q(wear_availability=True)).order_by("-wear_release_date")[:10]
    return render(request, "index.html", {'latest_mobiles': latest_mobiles, 'budget_mobiles': budget_mobiles,
                                          'articles_list': articles_list, 'ear_wear_list': ear_wear_list, 'wear_wear_list': wear_wear_list, })
...
def mobiles_details(request, brand_url, mobile_url):
    ...
    if mobile_details:
        return render(request, "mobiles/mobiledetails.html", {'mobile_details': mobile_details, 'variant': variant, 'rea_mobiles':rea_mobiles})
    else:
...
def mobiles_details_amp(request, brand_url, mobile_url):
    ...
    rea_mobiles = random.sample(list(rea_mobiles_list), 5)
    if mobile_details:
        return render(request, "mobiles/mobiledetailsamp.html", {'mobile_details': mobile_details, 'variant': variant, 'rea_mobiles':rea_mobiles})
    else:
        return render(request, "404.html", )
...
def mobiles_list(request, listmobile):
    ...
    if listmobile in static_a:
        ...
        return render(request, "mobiles/mobilelist.html", {'mobile_list': mobile_list, 'heading': heading, 'title': title, 'description': description, })
